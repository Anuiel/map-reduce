...
def dump_to_tmp_file(data: list[dict[str, tp.Any]]) -> str:
...
def text_input_file() -> str:
    ...
    return dump_to_tmp_file(data)


def test_word_count(text_input_file: str) -> None:
    runner = CliRunner()
    result = runner.invoke(run_word_count, ['--input', text_input_file])
    ...
    cmp_func = _Key('count', 'text')
    assert result.exit_code == 0
    output = list(map(ast.literal_eval, result.output.split('\n')[:-1]))
    assert sorted(output, key=cmp_func) == sorted(expected, key=cmp_func)


def test_inverted_index_graph(text_input_file: str) -> None:
    runner = CliRunner()
    result = runner.invoke(run_inverted_index_graph, ['--input', text_input_file])

    expected = [
        {'doc_id': 1, 'text': 'hello', 'tf_idf': approx(0.1351, 0.001)},
        {'doc_id': 6, 'text': 'hello', 'tf_idf': approx(0.1351, 0.001)},
        ...
        {'doc_id': 6, 'text': 'world', 'tf_idf': approx(0.2703, 0.001)}
    ]
    assert result.exit_code == 0
    cmp_func = _Key('count', 'text', 'tf_idf')
    output = list(map(ast.literal_eval, result.output.split('\n')[:-1]))
    assert sorted(output, key=cmp_func) == sorted(expected, key=cmp_func)


def test_pmi(text_input_file: str) -> None:
    runner = CliRunner()
    result = runner.invoke(run_pmi_graph, ['--input', text_input_file])
...
@pytest.fixture(scope="session")
def yandex_map_edge_lenght_file() -> str:
    data = [
...
@pytest.fixture(scope="session")
def yandex_map_travel_time_file() -> str:
    data = [
...
def test_yandex_map(yandex_map_edge_lenght_file: str, yandex_map_travel_time_file: str) -> None:
    runner = CliRunner()

    result = runner.invoke(run_yandex_maps_graph, [
        '--input-time', yandex_map_travel_time_file,
        '--input-road-lenght', yandex_map_edge_lenght_file
    ])
    ...
    cmp_func = _Key('weekday', 'hour', 'speed')
    assert result.exit_code == 0
    output = list(map(ast.literal_eval, result.output.split('\n')[:-1]))
    assert sorted(output, key=cmp_func) == sorted(expected, key=cmp_func)
